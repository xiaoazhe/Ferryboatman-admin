...
<script>
  import { FIND_MENU_TREE } from '@/api/modules/menu.js';
  import { ROLE_SAVE, ROLE_DELETE, ROLE_FIND_PAGE, ROLE_FIND_MENUS, ROLE_SAVE_MENUS } from '@/api/modules/role.js';
  ...
  export default {
    ...
    methods: {
      ...
      findPage: function (data) {
        ...
        ROLE_FIND_PAGE(this.pageRequest).then((res) => {
          this.pageResult = res
          this.findTreeData()
      ...
      handleDelete: function (data) {
        ROLE_DELETE(data.params).then(data.callback)
      },
      // 显示新增界面
      handleAdd: function () {
      ...
      handleRoleSelectChange (val) {
        ...
        ROLE_FIND_MENUS({ 'roleId': val.val.id }).then((res) => {
      ...
      submitAuthForm () {
        ...
        ROLE_SAVE_MENUS(roleMenus).then((res) => {
          if (res.code == 200) {
            this.$message({ message: '操作成功', type: 'success' })
          } else {
            this.$message({ message: '操作失败, ' + res.msg, type: 'error' })
